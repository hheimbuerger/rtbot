...
##        self.attribs[ "UID" ] = "NA"
    ...
    def toString(self):
        return self.name + ":" + self.attribs[ "Name" ] + " (" + str(self.attribs["UID"]) + ")"
...
class Ship(Buyable):
    def __init__(self,data):
        ...
        # read hardpoint data here
        
class Part(Buyable):
    def __init__(self, data):
        Buyable.__init__(self, data)
        self.offset += self.setAttribs( ["mass","sig","partID","nextID","type","mask"], "ffhhbH", data, self.offset )
...
#                self.treasures.append( Treasure( data[ offset : offset + tSize ] ) )
...
##
##    def filterByFaction(self, ents, faction):
##        return ents
##    
##        singleDefs = []
##        for s in faction.attribs["Def"].split():
##            if len( self.defMap[s] ) == 1:
##                singleDefs.append( s )
##
##        fList = []
##        for e in ents:
##            for s in singleDefs:
##                if s in e.attribs["Pre"]:
##                    fList.append( e )
##
##        return fList
##
